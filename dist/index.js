...
const sendOrdinal = async (paymentUtxo, ordinal, paymentPk, changeAddress, satPerByteFee, ordPk, ordDestinationAddress, reinscription, metaData) => {
    ...
    tx.add_output(changeOut);
    // sign ordinal
    const sig = tx.sign(ordPk, SigHash.InputOutput, 0, Script.from_asm_string(ordinal.script), BigInt(ordinal.satoshis));
    ordIn.set_unlocking_script(Script.from_asm_string(`${sig.to_hex()} ${ordPk.to_public_key().to_hex()}`));
    tx.set_input(0, ordIn);
    // sign fee payment
    const sig2 = tx.sign(paymentPk, SigHash.InputOutput, 1, Script.from_asm_string(paymentUtxo.script), BigInt(paymentUtxo.satoshis));
